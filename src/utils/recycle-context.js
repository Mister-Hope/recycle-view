...
RecycleContext.prototype.checkComp = function () {
  if (!this.comp) {
    ...
    if (this.comp) {
      ...
      throw new Error(
        "the recycle-view correspond to this context is detached, pls create another RecycleContext",
      );
    }
  }
};
  ...
  function newcb() {
    ...
    if (that.autoCalculateSize && that.placeholderClass) {
      ...
      wx.createSelectorQuery()
        ...
        .exec();
      wx.createSelectorQuery()
        .select("." + that.itemSizeOpt.queryClass)
        .boundingClientRect((rect) => {
          parentRect = rect;
          setPlaceholderImage();
        })
...
RecycleContext.prototype.splice = function (
  begin,
  deleteCount,
  appendList,
  cb,
...
// 将px转化为rpx
RecycleContext.prototype.transformRpx = RecycleContext.transformRpx = function (
  str,
  addPxSuffix,
) {
